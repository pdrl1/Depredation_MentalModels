# ============================================================
#  MENTAL MODEL ANALYSIS — REGIONAL SCALE
#  US Gulf Coast (USGC) community model
#
#  This script analyses the US Gulf Coast at the REGIONAL scale:
#    1) The AGGREGATED USGC community model (all workshops pooled
#       together, tag-independent) — including prominence.
#    2) Each of the Gulf Coast STATES separately (from the Tags
#       column): Alabama · Florida · Louisiana · Mississippi · Texas.
#
#  For every unit (aggregate + each state) this script computes:
...
#
#  This script supersedes the Gulf Coast portion of the previous
#  Goal1_GlobalComparison.R (aggregate model) and
#  Goal2_RegionalComparison.R (state-level breakdown) scripts, now
#  combined into a single regional-scale analysis.
# ============================================================
...
# ============================================================
# Kumu master export for the US Gulf Coast workshop (historically
# filed under "Alabama" -- it contains all five Gulf Coast states,
# distinguished via the Tags column).
# Strength column: signed integers +-1 / +-2 (back-converted from the
# Mental Modeler scale during standardisation).
# Tags column: pipe-separated state labels, e.g. "Texas|Louisiana"
...
cat("Loaded -- US Gulf Coast:", nrow(gc_elements), "elements |",
    nrow(gc_connections), "connections\n")

# Element group lookup table (for colouring plots) -- Gulf Coast uses 'Category'
...
# ============================================================
# SECTION 2 — UNIT DEFINITIONS (REGIONAL SCALE)
# ============================================================
# The "aggregate" unit = the full pooled USGC model (no Tags
# filtering). The state units mirror the Tags column exactly.

GC_STATES <- c("Alabama", "Florida", "Louisiana", "Mississippi", "Texas")

AGGREGATE_LABEL <- "US Gulf Coast"
GC_UNITS <- c(AGGREGATE_LABEL, GC_STATES)
...
# 3.2 Build a deduplicated edge list for one unit (aggregate or state).
...
#     directed eigenvector centrality is 0 for acyclic (DAG) state
#     networks (a real risk once we split into small states). Using
#     the undirected version everywhere -- aggregate AND states --
...
cat("\n\n========== US GULF COAST -- REGIONAL-SCALE METRICS ==========\n")
gc_metrics <- setNames(
...
gc_tbl <- make_metrics_table(gc_metrics, "US GULF COAST (AGGREGATE + STATES)")
...
# ---- Build a combined node table across ALL units (aggregate + states) ----
full_df_reg <- bind_rows(lapply(names(gc_metrics), function(u) {
  m <- gc_metrics[[u]]; if (is.null(m)) return(NULL)
  m$node_df %>% mutate(Unit = u)
})) %>%
  mutate(Unit = factor(Unit, levels = GC_UNITS)) %>%
...
# ---- 7.1 INDEGREE vs OUTDEGREE (AGGREGATE MODEL ONLY) ----

cat("Plot: Indegree vs Outdegree (US Gulf Coast, aggregated model only)...\n")

agg_df_reg <- full_df_reg %>%
  filter(Unit == AGGREGATE_LABEL) %>%
  # Lock Type to a factor with ALL type levels (even ones absent from this
  # unit's data) so scale_colour_manual(drop = FALSE) actually keeps a
  # consistent legend across the Indegree/Outdegree and Closeness/
  # Betweenness panels -- without this, a character column silently drops
  # unused levels and the two combined-panel legends can disagree.
...
io_label_pos  <- 0.90 * io_diag_max

p_gc_io <- ggplot(agg_df_reg, aes(x = Indegree, y = Outdegree,
  ...
  coord_cartesian(xlim = c(0,22), ylim = c(0,20)) +
  labs(title = "",
       x = "Indegree",
       y = "Outdegree",
       colour = "Concept type") +
...
ggsave("Regional_GC_indegree_vs_outdegree.png", p_gc_io,
       width = 9, height = 8, units = "in", dpi = 1200)
ggsave("Regional_GC_indegree_vs_outdegree.pdf", p_gc_io,
       width = 10, height = 9, units = "in", device = cairo_pdf)
ggsave("Regional_GC_indegree_vs_outdegree.tiff", p_gc_io,
       width = 10, height = 9, units = "in", dpi = 600, compression = "lzw")


# ---- 7.2 CLOSENESS vs BETWEENNESS (AGGREGATE MODEL ONLY) ----

cat("Plot: Closeness vs Betweenness (US Gulf Coast, aggregated model only)...\n")

p_keep_n <- 10   # label the top-10 by closeness OR betweenness

plot_df_cb <- agg_df_reg %>%
...
quad_lines_gc <- plot_df_cb %>%
  summarise(med_closeness   = median(Closeness,   na.rm = TRUE),
            med_betweenness = median(Betweenness, na.rm = TRUE))

p_gc_cb <- ggplot(plot_df_cb, aes(x = Closeness, y = Betweenness)) +
  geom_vline(data = quad_lines_gc, aes(xintercept = med_closeness),
             linetype = "dashed", colour = "grey70", linewidth = 0.4) +
  geom_hline(data = quad_lines_gc, aes(yintercept = med_betweenness),
             linetype = "dashed", colour = "grey70", linewidth = 0.4) +
...
ggsave("Regional_GC_closeness_vs_betweenness.png", p_gc_cb,
       width = 9, height = 8, units = "in", dpi = 1200)
ggsave("Regional_GC_closeness_vs_betweenness.pdf", p_gc_cb,
       width = 10, height = 9, units = "in", device = cairo_pdf)
ggsave("Regional_GC_closeness_vs_betweenness.tiff", p_gc_cb,
       width = 10, height = 9, units = "in", dpi = 600, compression = "lzw")
...
prom_Gc_rank <- plot_prominence_rank(
  gc_metrics, "", groups_df = gc_groups,
  col_order = GC_UNITS)

ggsave("Regional_GC_prominence_rank.png", prom_Gc_rank,
       width = 18, height = 11, dpi = 1200)
ggsave("Regional_GC_prominence_rank.pdf", prom_Gc_rank,
       width = 20, height = 12, device = cairo_pdf)

cat("\n\nRegional-scale (US Gulf Coast) analysis complete. All outputs saved.\n")
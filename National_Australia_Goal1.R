# ============================================================
#  MENTAL MODEL ANALYSIS — NATIONAL SCALE
#  Australia community model
#
#  This script analyses Australia at the NATIONAL scale:
#    1) The AGGREGATED Australia community model (all workshops
#       pooled together, tag-independent) — including prominence.
#    2) Each of the Australian REGIONS separately (from the Tags
#       column): Western Australia · North Australia · Queensland ·
#       New South Wales.
#
#  For every unit (aggregate + each region) this script computes:
...
#
#  This script supersedes the Australia portion of the previous
#  Goal1_GlobalComparison.R (aggregate model) and
#  Goal2_RegionalComparison.R (regional breakdown) scripts, now
#  combined into a single national-scale analysis.
# ============================================================
...
# ============================================================
# Kumu master export for Australia.
# Strength column: signed integers +-1 / +-2.
# Tags column: pipe-separated region labels,
...
cat("Loaded -- Australia:", nrow(au_elements), "elements |",
    nrow(au_connections), "connections\n")

# Element group lookup table (for colouring plots) - Australia uses 'Type'
...
# ============================================================
# SECTION 2 — UNIT DEFINITIONS (NATIONAL SCALE)
# ============================================================
# The "aggregate" unit = the full pooled Australia model (no Tags
# filtering). The region units mirror the Tags column exactly.
...
# 3.2 Build a deduplicated edge list for one unit (aggregate or region).
...
#     directed eigenvector centrality is 0 for acyclic (DAG) region
#     networks (a real risk once we split into small regions). Using
#     the undirected version everywhere -- aggregate AND regions --
...
cat("\n\n========== AUSTRALIA -- NATIONAL-SCALE METRICS ==========\n")
au_metrics <- setNames(
...
au_tbl <- make_metrics_table(au_metrics, "AUSTRALIA (AGGREGATE + REGIONS)")
...
# ---- Build a combined node table across ALL units (aggregate + regions) ----
full_df_nat <- bind_rows(lapply(names(au_metrics), function(u) {
  m <- au_metrics[[u]]; if (is.null(m)) return(NULL)
  m$node_df %>% mutate(Unit = u)
})) %>%
  mutate(Unit = factor(Unit, levels = AU_UNITS)) %>%
...
cat("Plot: Indegree vs Outdegree (Australia, aggregated model only)...\n")

agg_df_nat <- full_df_nat %>%
...
io_label_pos  <- 0.65 * io_diag_max

p_au_io <- ggplot(agg_df_nat, aes(x = Indegree, y = Outdegree,
  ...
  labs(title = "Indegree vs Outdegree",
       x = "Indegree",
       y = "Outdegree",
       colour = "Concept type") +
  theme_fcm() +
...
ggsave("National_AU_indegree_vs_outdegree.png", p_au_io,
       width = 9, height = 8, units = "in", dpi = 1200)
ggsave("National_AU_indegree_vs_outdegree.pdf", p_au_io,
       width = 10, height = 9, units = "in", device = cairo_pdf)
ggsave("National_AU_indegree_vs_outdegree.tiff", p_au_io,
       width = 10, height = 9, units = "in", dpi = 600, compression = "lzw")


# ---- 7.2 CLOSENESS vs BETWEENNESS (AGGREGATE MODEL ONLY) ----

cat("Plot: Closeness vs Betweenness (Australia, aggregated model only)...\n")

p_keep_n <- 10   # label the top-10 by closeness OR betweenness

plot_df_cb <- agg_df_nat %>%
...
quad_lines_au <- plot_df_cb %>%
  summarise(med_closeness   = median(Closeness,   na.rm = TRUE),
            med_betweenness = median(Betweenness, na.rm = TRUE))

p_au_cb <- ggplot(plot_df_cb, aes(x = Closeness, y = Betweenness)) +
  geom_vline(data = quad_lines_au, aes(xintercept = med_closeness),
             linetype = "dashed", colour = "grey70", linewidth = 0.4) +
  geom_hline(data = quad_lines_au, aes(yintercept = med_betweenness),
             linetype = "dashed", colour = "grey70", linewidth = 0.4) +
  ...
  scale_x_sqrt() +
  scale_y_sqrt() +
  labs(title = "Closeness vs Betweenness",
       x = "Closeness",
       y = "Betweenness",
       colour = "Concept type") +
  theme_fcm() +
...
ggsave("National_AU_closeness_vs_betweenness.png", p_au_cb,
       width = 9, height = 8, units = "in", dpi = 1200)
ggsave("National_AU_closeness_vs_betweenness.pdf", p_au_cb,
       width = 10, height = 9, units = "in", device = cairo_pdf)
ggsave("National_AU_closeness_vs_betweenness.tiff", p_au_cb,
       width = 10, height = 9, units = "in", dpi = 600, compression = "lzw")
...
cat("Plot: Prominence ranking (Australia, all units)...\n")
...
prom_Aus_rank <- plot_prominence_rank(
  au_metrics, "", groups_df = au_groups,
  col_order = AU_UNITS)

ggsave("National_AU_prominence_rank.png", prom_Aus_rank,
       width = 16, height = 11, dpi = 1200)
ggsave("National_AU_prominence_rank.pdf", prom_Aus_rank,
       width = 18, height = 12, device = cairo_pdf)

cat("\n\nNational-scale (Australia) analysis complete. All outputs saved.\n")